...
import {
  ...
  HardDrive, Box, Wifi, WifiOff, ExternalLink, RefreshCw
} from "lucide-react";
  ...
  return (
    <div className={cn("rounded-lg border border-border bg-muted/30 p-4 space-y-3", className)}>
      ...
      <p className="text-xs text-muted-foreground">
        ...
        The worker handles TRELLIS.2 inference and sends results back to the Wizard.
      </p>

      {workerUrl && (
        <div className="text-xs text-muted-foreground font-mono bg-muted/30 rounded px-2 py-1">
          Configured URL: {workerUrl}
        </div>
      )}
      ...
      <div className="text-xs text-muted-foreground space-y-1 border-t border-border pt-2">
        ...
        <p className="mt-1">
          VirtualBox: Use <code className="bg-muted px-1 rounded">http://10.0.2.2:8080</code> (NAT) or
          your Windows LAN IP (Bridged).
        </p>
      </div>
    </div>
  );
